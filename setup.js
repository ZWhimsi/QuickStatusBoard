...
async function setupFirebase() {
  ...
  const authDomain = await question(
...
export default app;`;
  ...
  fs.writeFileSync(
    path.join(__dirname, "config", "firebase.js"),
    firebaseConfig
  );
...
async function setupWeatherAPI() {
  ...
  const enableWeather = await question(
    "Do you want to set up weather API now? (y/n): "
  );

  if (enableWeather.toLowerCase() === "y") {
    const apiKey = await question("OpenWeatherMap API Key: ");

    const weatherConfig = fs.readFileSync(
      path.join(__dirname, "config", "weather.js"),
      "utf8"
    );
    const updatedConfig = weatherConfig.replace(
    ...
    console.log(
      "⏭️ Skipping weather API setup. You can configure it later in config/weather.js"
    );
  }
}
...
async function setupNotifications() {
  ...
  const enableNotifications = await question(
    "Do you want to set up push notifications now? (y/n): "
  );

  if (enableNotifications.toLowerCase() === "y") {
    const projectId = await question("Expo Project ID: ");

    const notificationConfig = fs.readFileSync(
      path.join(__dirname, "config", "notifications.js"),
      "utf8"
    );
    const updatedConfig = notificationConfig.replace(
    ...
    console.log(
      "⏭️ Skipping push notifications setup. You can configure it later in config/notifications.js"
    );
  }
}
